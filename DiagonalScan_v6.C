...
/*
格子のxy平面の側面にzは固定する
v5:複数の軸でpeakpositionを比較していく
v6:TFitオブジェクトを使い回していたのを修正→グラフごとに定義しなおす
   TFileでTGraphオブジェクトを保存
*/
...
tuple <double,double,double,double> interpolated_Bfield(double x, double y, double z){
    ...
    TFile *file = TFile::Open("/Users/shohtatakami/physics/COMETDS/DS189A_944turns_Integration/DS189A_944turns_FieldIntegration_Map.root");
...
int main(int argc, char** argv) {
    ...
    */
    ...
    string rootoutput_dir = "/Users/shohtatakami/physics/COMETDS/DS189A_944turns_Integration/Diagonal_Scan_Simulation/";//TFileオブジェクトの保存先
    //string rootoutput_dir = "/Users/shohtatakami/physics/COMETDS/DS189A_944turns/Diagonal_Scan_Simulation/";
    cxneg760->cd();
    TGraph *gxneg760 = new TGraph(z_neg760.size(), &z_neg760[0], &B_neg760[0]);
    string titleneg760 = "x=-760 : x1 = " + to_string(x1) + "x2 = " + to_string(x2);
    string rootneg760filename = "x_neg760__x1_" + to_string(x1) + "__" + "x2_" + to_string(x2);
    axis.push_back(-760);//x軸の位置を格納
    TF1 *quad_xneg760= new TF1("quadfit", "[0]*(x-[1])^2+[2]",-150,150);
    quad_xneg760->SetParameters(0,0.0,0.85);
    quad_xneg760->SetLineColor(kCyan);
    quad_xneg760->SetLineWidth(2);
    ...
    gxneg760->Draw("AP");
    gxneg760->Fit(quad_xneg760,"R");
    double p1neg760 = quad_xneg760->GetParameter(1);
    double p1neg760_err = quad_xneg760->GetParError(1);
    p1.push_back(p1neg760);
    p1_err.push_back(p1neg760_err);
    cxneg760->SetGridx();
    cxneg760->SetGridy();
    cxneg760->Update();
    auto xneg760_File = new TFile(Form("%s%s_out.root", rootoutput_dir.c_str(), rootneg760filename.c_str()),"RECREATE"); //オブジェクトをrootで保存
    cxneg760->Write();
    xneg760_File->Close();

    c0->cd();
    TGraph *g0 = new TGraph(z.size(), &z[0], &B[0]);
    string titlecenter = "x=+0 : x1 = " + to_string(x1) + "x2 = " + to_string(x2);
    string rootcenterfilename = "x_0__x1_" + to_string(x1) + "_" + "x2_" + to_string(x2);
    axis.push_back(0);//x軸の位置を格納
    TF1 *quadcenter = new TF1("quadfit", "[0]*(x-[1])^2+[2]",-150,150);
    quadcenter->SetParameters(0,0.0,0.85);
    quadcenter->SetLineColor(kCyan);
    quadcenter->SetLineWidth(2);
    g0->SetTitle("center");
    g0->SetMarkerStyle(6); 
    ...
    g0->Draw("AP");
    g0->Fit(quadcenter,"R");
    double p1center = quadcenter->GetParameter(1);
    double p1center_err = quadcenter->GetParError(1);
    p1.push_back(p1center);
    p1_err.push_back(p1center_err);
    c0->SetGridx();
    c0->SetGridy();
    c0->Update();
    auto centerFile = new TFile(Form("%s%s_out.root", rootoutput_dir.c_str(), rootcenterfilename.c_str()),"RECREATE"); //オブジェクトをrootで保存
    c0->Write();
    centerFile->Close();
    ...
    string rootposi500_filename = "x_500__x1_" + to_string(x1) + "__" + "x2_" + to_string(x2);
    axis.push_back(500);//x軸の位置を格納
    TF1 *quad_xposi500= new TF1("quadfit", "[0]*(x-[1])^2+[2]",-150,150);
    quad_xposi500->SetParameters(0,0.0,0.85);
    quad_xposi500->SetLineColor(kCyan);
    quad_xposi500->SetLineWidth(2);
    ...
    gxposi500->Draw("AP");
    gxposi500->Fit(quad_xposi500,"R");
    double p1posi500 = quad_xposi500->GetParameter(1);
    double p1posi500_err = quad_xposi500->GetParError(1);
    p1.push_back(p1posi500);
    p1_err.push_back(p1posi500_err);
    cxposi500->SetGridx();
    cxposi500->SetGridy();
    cxposi500->Update();
    auto xposi500File = new TFile(Form("%s%s_out.root", rootoutput_dir.c_str(), rootposi500_filename.c_str()),"RECREATE"); //オブジェクトをrootで保存
    cxposi500->Write();
    xposi500File->Close();
    ...
    string rootposi760filename = "x_760__x1_" + to_string(x1) + "__" + "x2_" + to_string(x2);
    axis.push_back(760);//x軸の位置を格納
    TF1 *quad_xposi760= new TF1("quadfit", "[0]*(x-[1])^2+[2]",-150,150);
    quad_xposi760->SetParameters(0,0.0,0.85);
    quad_xposi760->SetLineColor(kCyan);
    quad_xposi760->SetLineWidth(2);
    ...
    gxposi760->Draw("AP");
    gxposi760->Fit(quad_xposi760,"R");
    double p1posi760 = quad_xposi760->GetParameter(1);
    double p1posi760_err = quad_xposi760->GetParError(1);
    p1.push_back(p1posi760);
    p1_err.push_back(p1posi760_err);
    cxposi760->SetGridx();
    cxposi760->SetGridy();
    cxposi760->Update();
    auto xposi760File = new TFile(Form("%s%s_out.root", rootoutput_dir.c_str(), rootposi760filename.c_str()),"RECREATE"); //オブジェクトをrootで保存
    cxposi760->Write();
    xposi760File->Close();
    ...
    string rootneg500filename = "x_neg500__x1_" + to_string(x1) + "__" + "x2_" + to_string(x2);
    axis.push_back(-500);//x軸の位置を格納
    TF1 *quad_xneg500= new TF1("quadfit", "[0]*(x-[1])^2+[2]",-150,150);
    quad_xneg500->SetParameters(0,0.0,0.85);
    quad_xneg500->SetLineColor(kCyan);
    quad_xneg500->SetLineWidth(2);
    ...
    gxneg500->Draw("AP");
    gxneg500->Fit(quad_xneg500,"R");
    double p1neg500 = quad_xneg500->GetParameter(1);
    double p1neg500_err = quad_xneg500->GetParError(1);
    p1.push_back(p1neg500);
    p1_err.push_back(p1neg500_err);
    cxneg500->SetGridx();
    cxneg500->SetGridy();
    cxneg500->Update();
    auto xneg500File = new TFile(Form("%s%s_out.root", rootoutput_dir.c_str(), rootneg500filename.c_str()),"RECREATE"); //オブジェクトをrootで保存
    cxneg500->Write();
    xneg500File->Close();
